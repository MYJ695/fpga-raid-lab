...
RAID5 的核心只有一句话记住：**像 RAID0 一样把数据拆到多块盘，再给每个 stripe 加一块 XOR 校验块**。
...
> 本页先只讲 full-stripe write：一次写满一个 stripe 的所有数据块。partial write / write hole 会在后面页面单独拆。
...
RAID5 会让 parity 在不同成员盘之间轮转。现在 Python 模型的规则很直接：
```python
...
这里的 `P` 是现在 stripe 的 XOR parity，不是某个固定逻辑块。`D0`、`D1` 这些标签是 demo 里为了方便人眼观察而写的逻辑数据块编号。
...
现在 Level 0 模型故意只实现 full-stripe write：一次写满一个 stripe 的所有数据块。
...
⬅️ [上一篇：RAID1 镜像](raid1_mirror.md)<br>
🏠 [回到课程目录](index.md)<br>
