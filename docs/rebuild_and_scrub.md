...
RAID5 的 write hole 最危险之处，是正常读可能看不出来。**Rebuild** 和 **scrub** 是两个会把潜伏 mismatch 逼出来的维护动作：

1. rebuild：某块盘坏了，用剩余 data/parity 重建缺失块；
2. scrub：盘还没坏，主动扫描 stripe，重新计算 parity，检查磁盘上的 parity 是否一致；
3. 对 FPGA RAID 控制器来说，它们都不是“神秘算法”，而是按 stripe 循环读、XOR、比较、写回或上报。

一句话：**write path 负责制造一致 stripe；rebuild/scrub 负责在未来证明 stripe 仍然一致。**
...
Scrub 可以理解为“RAID 自检巡逻”。它不等盘坏，而是定期扫描 stripe：
...
把 rebuild/scrub 映射到 FPGA，不需要一开始就做完整控制器。可以先抽成几个小模块和状态：
