...
RAID0 的核心只有一句话记住：**把连续的逻辑块轮流发给多块盘**。

如果有 `disk_count` 块盘，而且每个 chunk 在本实验里等于 1 个 block，那么映射公式就是：
```text
...
现在实现位于 [`../labs/level0_python_model/raid_model.py`](../labs/level0_python_model/raid_model.py)：
```python
...
⬅️ [上一篇：RAID 基础](raid_basics.md)<br>
🏠 [回到课程目录](index.md)<br>
